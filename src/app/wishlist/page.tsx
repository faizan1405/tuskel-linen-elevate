    ...
    <div className="shell pb-24">
      ...
      {!hydrated ? (
        ...
        <div className="mt-12 border border-dashed border-border px-6 py-20 text-center">
          ...
          <Link href="/shop" className="mt-7 inline-block min-h-12 bg-foreground px-8 py-3.5 text-[11px] tracking-[0.18em] text-primary-foreground uppercase">Shop the collection</Link>
        </div>
      ) : (
            ...
            <li key={p.slug} className="flex flex-col gap-4 py-6 sm:flex-row">
              ...
              <div className="flex-1">
                <div className="flex justify-between gap-3">
                  <div>
                    <Link href={`/product/${p.slug}`} className="text-[14px] font-medium link-underline">{p.name}</Link>
