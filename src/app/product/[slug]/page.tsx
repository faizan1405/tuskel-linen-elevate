...
export default function ProductPage(props: any) {
  const { product } = React.use((props as any).loaderData) as any; // Next.js loader data needs to be passed via props
          ...
          <div className="lg:sticky lg:top-28 lg:h-fit">
            ...
            <div className="mt-8">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {products.filter((p) => p.fabric === product.fabric).map((p) => (
                  <Link key={p.slug} href={`/product/${p.slug}`} aria-label={p.colorName} title={p.colorName}
            ...
            <Accordion type="single" collapsible className="mt-8 border-t border-border">
              <AccordionItem value="details"><AccordionTrigger className="text-[13px]">Product details</AccordionTrigger>
                <AccordionContent><ul className="list-disc space-y-1.5 pl-5 text-[13.5px] text-muted-foreground">{product.details.map((d: any) => <li key={d}>{d}</li>)}</ul>
              ...
              <AccordionItem value="care"><AccordionTrigger className="text-[13px]">Fabric and care</AccordionTrigger>
                <AccordionContent><ul className="list-disc space-y-1.5 pl-5 text-[13.5px] text-muted-foreground">{product.care.map((d: any) => <li key={d}>{d}</li>)}</ul></AccordionContent></AccordionItem>
