...
export const Route = createFileRoute("/checkout")({
  head: () => ({
    meta: [
      { title: "Checkout — Tuskel" },
      { name: "description", content: "Complete your Tuskel order securely." },
      { property: "og:title", content: "Checkout — Tuskel" },
      { property: "og:description", content: "Complete your Tuskel order securely." },
  ...
  if (placed) {
    return (
      <div className="shell flex min-h-[60vh] flex-col items-center justify-center py-24 text-center">
        ...
        <Link to="/shop" className="mt-8 min-h-12 bg-foreground px-8 py-3.5 text-[11px] tracking-[0.18em] text-primary-foreground uppercase">Back to Shop</Link>
      </div>
    );
  }
      ...
      <div className="mt-10 grid gap-14 lg:grid-cols-[1.3fr_1fr] lg:gap-20">
        ...
        <aside className="lg:sticky lg:top-28 lg:h-fit">
          ...
          <ul className="divide-y divide-border border-y border-border">
            {lines.length === 0 && <li className="py-6 text-[13px] text-muted-foreground">Your bag is empty. <Link to="/shop" className="underline">Add a shirt</Link>.</li>}
            {lines.map((l) => (
