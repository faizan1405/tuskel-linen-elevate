...
function OrderDetailModal({ order, onClose }: { order: AdminOrder; onClose: () => void }) {
  ...
  const advanceStatus = () => {
    ...
    if (idx < ORDER_STATUS_FLOW.length - 1) setStatus(ORDER_STATUS_FLOW[idx + 1]);
  };
...
export const Route = createFileRoute("/admin/orders")({
  head: () => ({ meta: [{ title: "Orders — Tuskel Admin" }, { name: "robots", content: "noindex" }] }),
                ...
                {isLoading ? (
                  ...
                  filtered.map((o) => (
                    <TableRow key={o.id}>
                      ...
                      <TableCell className="text-sm">{o.items.reduce((s, it) => s + it.qty, 0)} items</TableCell>
